...
trait HyperionCli { this: DataPipelineDef =>
  ...
  def main(args: Array[String]): Unit = {
    val parser = new OptionParser[Cli](s"hyperion") {
      ...
      cmd("create").action { (_, c) => c.copy(mode = "create") }
        .children(
          ...
          opt[String]('n', "name").valueName("<name>")
            .action { (x, c) => c.copy(customName = Option(x)) },
          opt[String]("region").valueName("<region>")
            .action { (x, c) => c.copy(region = Option(x)) },
          opt[String]("role").valueName("<role-arn>")
            .action { (x, c) => c.copy(roleArn = Option(x)) },
          opt[(String, String)]('t', "tags").valueName("<tag>")
            .action { (x, c) =>
              val tag = x match {
                case (k, "") => (k, None)
                case (k, v) => (k, Option(v))
            ...
            .unbounded(),
          opt[(String, String)]('p', "params").valueName("<params>")
            .text("aws pipeline parameters (e.g. myKey1=myValue1,myKey2=myValue2")
            .action { (x, c) =>
              c.copy(params = c.params + x)
            }
            .unbounded()
        )
    ...
    parser.parse(args, Cli()).map { cli =>
      ...
      val pipelineDef = cli.params match {
        case xs if xs.isEmpty => this
        case xs => this.overwriteParameters(xs.map { case (i, v) => StringParameter(i, v) })
      }
